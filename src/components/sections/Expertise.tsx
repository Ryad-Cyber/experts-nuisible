...
export function Expertise() {
  return (
    <Section id="services" variant="muted" className="relative overflow-hidden">
      ...
      <StaggerGroup className="relative mt-10 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-6 lg:grid-rows-2">
        {services.map((service) => {
          const Icon = service.icon;
          const isFeature = service.id === "deratisation";

          return (
            <StaggerItem key={service.id} className={cn("min-h-[8rem]", SPANS[service.id])}>
              <div
                className={cn(
                  "group relative flex h-full flex-col justify-between overflow-hidden rounded-2xl border p-5 transition-all duration-300",
                  isFeature
                ...
                <div className="relative flex items-start justify-between gap-3">
                  <span
                    className={cn(
                      ...
                      isFeature ? "size-12 bg-white/10 text-accent-light" : "size-10 bg-secondary/10 text-secondary"
                    )}
                  >
                    <Icon className={isFeature ? "size-6" : "size-5"} />
                  </span>
                  <ArrowUpRight
                    className={cn(
                      "size-4 -translate-y-0.5 translate-x-0.5 opacity-0 transition-all duration-300 group-hover:translate-x-0 group-hover:translate-y-0 group-hover:opacity-100",
      ...
      <Reveal delay={0.1} className="relative mt-8 flex justify-center">
